    ...
    pub async fn login_runner(payload: &Login, data: &AppData) -> ServiceResult<String> {
        ...
        if payload.login.contains('@') {
            ...
            let email_fut = sqlx::query_as!(
                EmailLogin,
                r#"SELECT name, password  FROM kaizen_users WHERE email = ($1)"#,
                &payload.login,
            )
            .fetch_one(&data.db)
            .await;
            ...
            let username_fut = sqlx::query_as!(
                Password,
                r#"SELECT password  FROM kaizen_users WHERE name = ($1)"#,
                &payload.login,
            )
            .fetch_one(&data.db)
            .await;
        ...
        (name , password,  secret) VALUES ($1, $2, $3)",
            ...
            if res.is_ok() {
                ...
                if err.code() == Some(Cow::from("23505")) {
                    let msg = err.message();
                    if msg.contains("kaizen_users_name_key") {
                        return Err(ServiceError::UsernameTaken);
                    } else if msg.contains("kaizen_users_email_key") {
                        return Err(ServiceError::EmailTaken);
                    } else if msg.contains("kaizen_users_secret_key") {
                        continue;
                    } else {
